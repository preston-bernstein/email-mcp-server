...
5. **Verify the tool appears** — depends on transport (procedures owned by `proton-mcp-run-and-operate`; summary):
   ...
   - **streamable-http (desktop Docker):** the running container was built from an image dated 2026-06-20 — editing the Mac repo changes NOTHING in prod. You must rebuild the Docker image, restart the container, and restart LibreChat so it re-handshakes the MCP server. Beware source drift: two desktop copies of the source exist besides the Mac repo (`/home/preston/docker/proton-email-mcp`, `<docker-root>/librechat-stack/proton-email-mcp`); the Mac git repo is canonical — edit there first, then propagate.
