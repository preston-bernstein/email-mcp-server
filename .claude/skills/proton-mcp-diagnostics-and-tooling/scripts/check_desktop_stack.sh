#!/usr/bin/env bash
# check_desktop_stack.sh — is the email stack up on the desktop (desktop.example.internal)?
#
# Via `ssh desktop.example.internal` (read-only), checks:
...
#   SSH_TARGET=desktop.example.internal ./check_desktop_stack.sh
#
