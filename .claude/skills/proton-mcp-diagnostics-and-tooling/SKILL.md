...
- **Desktop** — the Linux box at `10.0.0.243`, reachable read-only as `ssh desktop-agent`
...
| `scripts/drift_check.sh` | Do the 3 source copies still match? | `ssh desktop-agent` |
| `scripts/check_desktop_stack.sh` | Are the desktop containers up and ports listening? | `ssh desktop-agent` |
...
# override: BRIDGE_HOST=127.0.0.1 ... (defaults: BRIDGE_HOST=10.0.0.243, MCP_PORT=3004)
```
...
Bridge host: 10.0.0.243 | MCP host: 10.0.0.243

PASS  Bridge IMAP                  10.0.0.243:1143 open
PASS  Bridge SMTP                  10.0.0.243:1025 open
PASS  MCP streamable-http          10.0.0.243:3004 open
PASS  MCP HTTP liveness            http://10.0.0.243:3004/mcp answered HTTP 421 (any status = server ALIVE; 4xx expected for GET)
...
|---|---|---|---|---|---|
| FAIL | FAIL | FAIL | FAIL | Network / desktop host down (or wrong `BRIDGE_HOST`) | `ping 10.0.0.243`; run `check_desktop_stack.sh` if SSH works |
...
| Desktop A | `/home/preston/docker/proton-email-mcp` | `ssh desktop-agent sudo sha256sum` |
| Desktop B | `/opt/docker/librechat-stack/proton-email-mcp` | `ssh desktop-agent sudo sha256sum` |
...
| Cannot ssh | No data — do not conclude anything about drift | Fix SSH first (`ssh desktop-agent true`) |

## 3. check_desktop_stack.sh — container and port health on the desktop

**Measures:** via `ssh desktop-agent`: `docker ps` filtered to
...
# optional: PROTON_BRIDGE_HOST (default 10.0.0.243), PROTON_BRIDGE_IMAP_PORT (1143), SMOKE_FOLDER (INBOX)
```
...
```
Connecting to 10.0.0.243:1143 (plain IMAP, socket timeout 15s)...
