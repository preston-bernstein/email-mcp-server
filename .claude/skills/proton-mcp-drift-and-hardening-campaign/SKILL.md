...
# Copy B vs Mac repo
ssh $PROTON_MCP_SSH_HOST sudo cat <docker-root>/librechat-stack/proton-email-mcp/proton_email_server.py \
  | diff - /Users/prestonbernstein/dev/proton-email-mcp/proton_email_server.py
```
...
`ssh $PROTON_MCP_SSH_HOST "sudo crontab -l -u preston; sudo grep -rn 'proton-email-mcp' <docker-root>/ /etc/systemd/system/ 2>/dev/null | grep -v Binary"`).
...
ssh $PROTON_MCP_SSH_HOST "sudo mkdir -p <docker-root>/librechat-stack/proton-email-mcp /home/preston/docker/proton-email-mcp \
...
```bash
ssh $PROTON_MCP_SSH_HOST "sudo docker build -t proton-email-mcp:latest <docker-root>/librechat-stack/proton-email-mcp"
```
...
locate with `ssh $PROTON_MCP_SSH_HOST "sudo find <docker-root>/auth-stack/data/protonmail-bridge -name '*.pem' 2>/dev/null"`;
...
1. `ssh $PROTON_MCP_SSH_HOST "sudo cat <docker-root>/librechat-stack/Caddyfile"` — learn
