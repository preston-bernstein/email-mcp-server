...
4. **Desktop access:** `ssh desktop.example.internal` (agent user, key `~/.ssh/agent_ed25519`,
...
6. **Standard validation gate** (referenced as "STANDARD GATE" below) — every phase
   ...
   - Read-only live smoke from the Mac:
     ...
     with `PROTON_BRIDGE_HOST=desktop.example.internal` and credentials sourced from
...
locations (desktop via `ssh desktop.example.internal sudo sha256sum`, read-only).
...
- Exit 1 with `FAIL: cannot ssh` → fix SSH first (`ssh desktop.example.internal true` must
...
ssh desktop.example.internal sudo cat /home/preston/docker/proton-email-mcp/proton_email_server.py \
  | diff - /Users/prestonbernstein/dev/proton-email-mcp/proton_email_server.py

# Copy B vs Mac repo
ssh desktop.example.internal sudo cat <docker-root>/librechat-stack/proton-email-mcp/proton_email_server.py \
  | diff - /Users/prestonbernstein/dev/proton-email-mcp/proton_email_server.py
```
...
**Recommended designated desktop build context: `<docker-root>/librechat-stack/proton-email-mcp` (copy B).**
...
`ssh desktop.example.internal "sudo crontab -l -u preston; sudo grep -rn 'proton-email-mcp' <docker-root>/ /etc/systemd/system/ 2>/dev/null | grep -v Binary"`).
...
ssh desktop.example.internal "sudo docker inspect proton-email-mcp | sudo tee /root/proton-email-mcp.inspect.$(date +%Y%m%d).json >/dev/null && sudo chmod 600 /root/proton-email-mcp.inspect.$(date +%Y%m%d).json"

# Credential-free summary you MAY read (redacts values of PROTON_* vars)
ssh desktop.example.internal "sudo docker inspect proton-email-mcp --format 'Image={{.Config.Image}} Net={{.HostConfig.NetworkMode}} Restart={{.HostConfig.RestartPolicy.Name}} Mounts={{range .Mounts}}{{.Source}}:{{.Destination}} {{end}}' && sudo docker inspect proton-email-mcp --format '{{range .Config.Env}}{{println .}}{{end}}' | sed -E 's/^(PROTON_USERNAME|PROTON_PASSWORD)=.*/\1=<REDACTED-present>/'"

# Env file for recreation (root-only, on desktop; values never leave the machine)
ssh desktop.example.internal "sudo docker inspect proton-email-mcp --format '{{range .Config.Env}}{{println .}}{{end}}' | sudo tee /root/proton-email-mcp.env >/dev/null && sudo chmod 600 /root/proton-email-mcp.env"
```
...
(`ssh desktop.example.internal "sudo docker inspect proton-email-mcp --format '{{index .Config.Labels \"com.docker.compose.project.working_dir\"}}'"`)
...
The agent user likely cannot write `<docker-root>/` or `/home/preston/` directly
...
rsync -av proton_email_server.py Dockerfile requirements.txt .env.example \
  desktop.example.internal:proton-email-mcp-staging/

ssh desktop.example.internal "sudo mkdir -p <docker-root>/librechat-stack/proton-email-mcp /home/preston/docker/proton-email-mcp \
  && sudo cp ~/proton-email-mcp-staging/* <docker-root>/librechat-stack/proton-email-mcp/ \
...
```bash
ssh desktop.example.internal "sudo docker build -t proton-email-mcp:latest <docker-root>/librechat-stack/proton-email-mcp"
```

**EXPECTED:** build succeeds; final line reports the image tagged. Then confirm
freshness:
`ssh desktop.example.internal "sudo docker inspect proton-email-mcp:latest --format '{{.Created}}'"`
...
```bash
ssh desktop.example.internal "sudo docker rm -f proton-email-mcp \
...
```bash
ssh desktop.example.internal "sudo docker ps --filter name=proton-email-mcp --format '{{.Status}}' && sudo docker logs proton-email-mcp --tail 20 2>&1 | grep -v -i password"
```
...
```bash
ssh desktop.example.internal "curl -s -o /dev/null -w '%{http_code}\n' -X POST http://localhost:3004/mcp \
...
```bash
ssh desktop.example.internal "sudo docker ps --format '{{.Names}}' | grep -i -E 'librechat|chat'"
ssh desktop.example.internal "sudo docker restart <discovered-name>"
ssh desktop.example.internal "sudo docker logs <discovered-name> --since 3m 2>&1 | grep -i -E 'proton|mcp' | head -20"
```
...
`ssh desktop.example.internal "sudo mv /home/preston/docker/proton-email-mcp /home/preston/docker/proton-email-mcp.DEPRECATED-$(date +%Y%m%d)"`.
...
```bash
ssh desktop.example.internal "sudo docker inspect <librechat-container-name> --format '{{.HostConfig.NetworkMode}}'"
```
...
- **Case 2 — bridge / a named network:** then `localhost` inside the LibreChat
  ...
  `<docker-root>/librechat-stack/docker-compose.yml` (filename UNVERIFIED — `ls` the
...
- From the Mac: `nc -z -w 3 desktop.example.internal 3004` → **FAILS** (non-zero exit).
...
```bash
openssl s_client -connect desktop.example.internal:1143 -starttls imap </dev/null
```
...
| Pin the Bridge cert | `load_verify_locations(<bridge-cert>.pem)`, `verify_mode=CERT_REQUIRED`, `check_hostname=False` (CN is UNVERIFIED, likely not desktop.example.internal); stronger — MITM-resistant | Preferred end state; adds maintenance (cert lives in the Bridge volume and changes if Bridge regenerates it) |

To pin: the cert lives in the Bridge data volume
`<docker-root>/auth-stack/data/protonmail-bridge` (exact filename UNVERIFIED —
locate with `ssh desktop.example.internal "sudo find <docker-root>/auth-stack/data/protonmail-bridge -name '*.pem' 2>/dev/null"`;
...
- Packet-level proof (optional but decisive): on the desktop,
  `ssh desktop.example.internal "sudo timeout 15 tcpdump -i any -c 20 -A port 1143 2>/dev/null | grep -c LOGIN"`
...
`<docker-root>/librechat-stack/` (verified present 2026-07-02; **contents
...
1. `ssh desktop.example.internal "sudo cat <docker-root>/librechat-stack/Caddyfile"` — learn
...
WITH the header → 200; `nc -z -w 3 desktop.example.internal 3004` from the Mac fails (B1 still
