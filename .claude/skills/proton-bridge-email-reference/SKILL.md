...
**Where Bridge runs here** (facts pack §5C, verified 2026-07-02): as a Docker container `protonmail-bridge` on the desktop ($PROTON_MCP_HOST), defined in `/opt/docker/auth-stack/docker-compose.yml`, publishing `1143:143` (IMAP) and `1025:25` (SMTP), with its state/auth in the volume `./data/protonmail-bridge:/root`. The MCP server reaches it via `PROTON_BRIDGE_HOST` / `PROTON_BRIDGE_IMAP_PORT` / `PROTON_BRIDGE_SMTP_PORT` (lines 31–33; defaults `127.0.0.1`, `1143`, `1025`).
