...
Docker container on the desktop (desktop.example.internal:3004) for LibreChat. One commit in history
...
|---|---|---|---|
| W1 | Plaintext IMAP over LAN | Mac deployment sets `PROTON_BRIDGE_HOST=desktop.example.internal`; `imaplib.IMAP4` is plaintext, so the Bridge password crosses the LAN unencrypted on every tool call. (Whether this Bridge build accepts STARTTLS on 1143 is UNVERIFIED.) | open |
...
| W7 | Vestigial requirements | `secure-smtplib`, `httpx`, `python-dotenv` are in requirements.txt but never imported; the server NEVER loads `.env` (config must come from the process environment — `.env.example` is a template only, and its `127.0.0.1` host does not match the real Mac deployment's `desktop.example.internal`). Removal changes the Docker build — change control. | open |
| W8 | 3-copy source drift | Same source exists at the Mac repo (canonical), `/home/preston/docker/proton-email-mcp`, and `<docker-root>/librechat-stack/proton-email-mcp` on the desktop. No deploy pipeline; prod image built 2026-06-20. Drift MEASURED 2026-07-02: the `/home/preston` copy is a stale pre-fix snapshot; the librechat-stack copy matched canonical (worked example in `proton-mcp-diagnostics-and-tooling`; re-measure before acting). Edit the Mac repo first, always. | open |
...
verified facts pack dated 2026-07-02. Facts about the desktop (desktop.example.internal) come from the
facts pack, not fresh SSH; re-verify them from a machine with `ssh desktop.example.internal` access.
...
| Desktop container config (host net, :3004) | from a machine with access: `ssh desktop.example.internal "docker inspect proton-email-mcp --format '{{.HostConfig.NetworkMode}} {{.Config.Env}}'"` (do not paste credential env values anywhere) |
| W8: source drift between the 3 copies | `ssh desktop.example.internal "md5sum /home/preston/docker/proton-email-mcp/proton_email_server.py <docker-root>/librechat-stack/proton-email-mcp/proton_email_server.py"` vs `md5 -q /Users/prestonbernstein/dev/proton-email-mcp/proton_email_server.py` |
