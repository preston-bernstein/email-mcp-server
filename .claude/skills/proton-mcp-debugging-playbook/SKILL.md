...
(compose file `<docker-root>/auth-stack/docker-compose.yml`, state volume
`<docker-root>/auth-stack/data/protonmail-bridge`).
...
2. **Bridge container running but not authenticated** (logged out, first-run not completed, or
   ...
   `<docker-root>/auth-stack/data/protonmail-bridge` on the desktop.
...
2. **`librechat.yaml` missing `mcpSettings.allowedDomains` entry `http://localhost:3004`.**
   ...
   required, verified present in `<docker-root>/librechat-stack/librechat.yaml` as of 2026-07-02
...
- **Container up, :3004 listening, tools still absent** → inspect
  `<docker-root>/librechat-stack/librechat.yaml` for both the `mcpServers.proton-email` block and
...
- **Three copies of the source exist** (Mac repo = canonical; desktop
  `/home/preston/docker/proton-email-mcp` and `<docker-root>/librechat-stack/proton-email-mcp`
