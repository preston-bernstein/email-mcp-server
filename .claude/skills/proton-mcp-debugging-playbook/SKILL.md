...
|---|---|---|---|
| Mac / Claude Code | stdio (server spawned as a subprocess, speaks MCP over stdin/stdout) | `~/.claude.json` → `mcpServers["proton-mail"]` | `10.0.0.243` (the desktop, over LAN) |
...
- Commands marked **[desktop]**: run from the desktop maintenance context via `ssh desktop-agent`.
...
- Call `search_emails` with a harmless query on the same folder.
  ...
  **[desktop]** (run from desktop maintenance context via `ssh desktop-agent`):
...
2. **Wrong `PROTON_BRIDGE_HOST`.** Known trap: `.env.example` says `127.0.0.1`, but the real Mac
   stdio deployment must use `10.0.0.243` (the desktop) — the example is a template, not a record
...
```
nc -vz 10.0.0.243 1143
nc -vz 10.0.0.243 1025
```
...
Discriminating experiment — **[desktop]** (via `ssh desktop-agent`):
...
| `.env.example` host divergence | Template says `PROTON_BRIDGE_HOST=127.0.0.1`; real Mac deployment uses `10.0.0.243` | `.env.example` line 3 vs `~/.claude.json` deployment | Standing trap; template ≠ deployment record |
...
- Desktop state **[desktop]** via `ssh desktop-agent`: `docker ps && ss -tln | grep -E "1143|1025|3004"`
